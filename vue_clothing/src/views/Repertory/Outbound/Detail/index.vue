<template>
  <div>
      <div id="h2">
        <el-button @click="backPage" class="iconfont icon-return1x" size="small" type="primary" plain></el-button>
        <el-button @click="saveInfo" v-if="isAdd" class="iconfont icon-icon_baocun" size="small" type="primary" plain></el-button>
      </div>
      ...
      <div id="info">
        <el-form ref="form" :model="dataInfo" label-width="80px" id="form">
            <el-form-item label="单据号" >
                <el-input v-model="dataInfo.id" placeholder="自动编号" disabled style="width:220px"></el-input>
            </el-form-item>
            <el-form-item label="接收人">
                <el-input v-model="dataInfo.recipient" style="width:220px"></el-input>
            </el-form-item>
            <el-form-item label="联系电话">
                <el-input v-model="dataInfo.recipientPhone" style="width:220px"></el-input>
            </el-form-item>
      ...
      <el-dialog title="新增明细" @close="closeDialog('commodityForm')" :visible.sync="dialogFormVisible" width="30%" center destroy-on-close>
          <el-form  :model="commodityForm" ref="commodityForm" :rules="rules" label-position="left" label-width="20%">
            ...
            <el-form-item label="品名" prop="commName" >
              <el-input v-model="commodityForm.commName" autocomplete="off" :disabled="autoGetInfo" placeholder="字符"></el-input>
            </el-form-item>
            <el-form-item label="色号" prop="commColor">
              <el-input v-model="commodityForm.commColor" autocomplete="off" :disabled="autoGetInfo" placeholder="字符"></el-input>
            </el-form-item>
            <el-form-item label="尺码" prop="commSize">
              <el-input v-model="commodityForm.commSize" autocomplete="off" :disabled="autoGetInfo" placeholder="字符"></el-input>
            </el-form-item>
    ...
    data() {
      ...
      //验证货号
        var checkId = (rule, value, callback) => {
            ...
            const isbnReg = /^\d{8}$/;
            if (value === '') {
            return callback(new Error('请输出货号'));
            } 
        ...
        var checkNumber = (rule, value, callback) => {
            ...
            const isbnReg = /^\d{1,}$/;
            if (value === '') {
            return callback(new Error('请输出数量'));
            } 
            if (!isbnReg.test(value)) {
        ...
        return {
            ...
            dataInfo: {id:'',date:'',operator:'',recipient:'',recipientPhone:'',remark:'',detail:[
                 // {id:'1',commName:'棉袄',commColor:'红色',commSize:'S',commNumber:'100'},
                //  {id:'2',commName:'棉袄',commColor:'红色',commSize:'S',commNumber:'200'},
    ...
    methods:{
            ...
            if(!this.checkInfo().isTrue){
             ...
             this.$confirm('是否保存信息?', '提示', {
                ...
                type: 'warning'
                    ...
                    this.$store.dispatch('outbound/addOutbound',this.dataInfo).then(data=>{
                         this.$alert('您的单据号为：'+data.outboundId, '提示', {
                            confirmButtonText: '确定',
                            callback: action => {
                                this.$router.back()
                            }
        ...
        updateInbound(){
            ...
            this.$confirm('你的操作将更新入库单信息，是否继续?', '提示', {
                ...
                type: 'warning'
                    ...
                    let params={
                        id:this.dataInfo.id,
                        recipient:this.dataInfo.recipient,
                        recipientPhone:this.dataInfo.recipientPhone,
                        date:this.dataInfo.date,
                        remark:this.dataInfo.remark,
                    }
                    this.$store.dispatch('outbound/updateOutbound',params).then(data=>{
        ...
        submitForm(){        
            this.$refs['commodityForm'].validate(valid => {
                if(valid){
                    if(!this.autoGetInfo)  return this.$message.error('该货品不存在，请输入正确的货号')
                    //详情增加模块(数组添加)
                    if(this.isAdd){
                        //添加详情
                        if(this.isAddDetail){
                    ...
                    if(this.isAddDetail){
                        ...
                        this.$store.dispatch('outbound/addOutboundDetail',data).then(data=>{
                            this.$message.success(data)
                            this.dialogFormVisible=false
                            this.queryDetail()
                        }).catch(err=>{
                        ...
                        let data = JSON.parse(JSON.stringify(this.commodityForm));
                        data["id"]=this.dataInfo.id
                        this.$store.dispatch('outbound/updateOutboundDetail',data).then(data=>{
                            this.$message.success(data)
                            this.dialogFormVisible=false
                            this.queryDetail()
                        }).catch(err=>{
        ...
        queryDetail(){
            ...
            this.$store.dispatch('outbound/queryDetail',this.dataInfo.id).then(res=>{
            ...
            this.$confirm('该操作将删除数据，是否继续?', '提示', {
                ...
                type: 'warning'
                    ...
                    if(this.isAdd){
                        ...
                        //请求方式删除明细
                        this.$store.dispatch('outbound/deleteDetail',row).then(res=>{
                            this.$message({
                                type: 'success',
                                message: res
                            });
        ...
        //页面信息非空检查
        checkInfo(){
            if(this.dataInfo.recipient=='') return {isTrue:false,msg:'接收人不能为空'}
            if(this.dataInfo.recipientPhone == '') return {isTrue:false,msg:'联系电话不能为空'}
            let reg =/^1[3-9]\d{9}$/
            if(!reg.test(this.dataInfo.recipientPhone)) return {isTrue:false,msg:'联系电话格式不正确'}
